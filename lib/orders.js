...
// Marks (or unmarks) a single order line item as a damaged-goods return.
// Returned items keep their order history (never deleted) but are excluded
// from the Billing profit margin calculation — see listBillingRows() below.
export function markItemReturned(itemId, { returned, reason } = {}) {
  ...
  if (returned) {
    db.prepare(
      "UPDATE order_items SET is_returned = 1, return_reason = ?, returned_at = ? WHERE id = ?"
    ).run(reason?.trim() || "Damaged goods return", new Date().toISOString(), itemId);
  } else {
    db.prepare(
      "UPDATE order_items SET is_returned = 0, return_reason = NULL, returned_at = NULL WHERE id = ?"
    ).run(itemId);
  }
...
export function listBillingRows({ from, to } = {}) {
  ...
  return rows.map((row) => {
    ...
    // Damaged returns are excluded from profit margin entirely, even when a
    // buying cost is on record — the sale didn't ultimately go through.
    const profitMargin =
      !isReturned && buyingCostTotal != null
        ? Math.round((totalSellingCost - buyingCostTotal) * 100) / 100
        : null;
